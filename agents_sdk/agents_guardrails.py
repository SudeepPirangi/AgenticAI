...
async def main():
    ...
    try:
        ...
        guardrail_agent = await gemini_ai.agent(
            name="Email Check",
            instructions="Check if the user is including someone's email address in what they want you to do.",
        ...
        async def guardrail_against_email(ctx, agent, message):
            ...
            print("email guardrail", result.final_output)
            is_email_in_message = result.final_output.is_email_in_message
            return GuardrailFunctionOutput(
                output_info={"found_email": result.final_output},
                tripwire_triggered=is_email_in_message,
            )

        sales_content_tools = [tool1, tool2, tool3]
        handoffs = [emailer_agent]
        email_guardrail = [guardrail_against_email]
        sales_manager_message = "Send a cold sales email address to the recipient"

        # Sales Manager agent who orchestrates all the process/workflow
        careful_sales_manager = await gemini_ai.agent(
            name="Sales Manager",
            instructions=sales_manager_instructions,
            tools=sales_content_tools,
            handoffs=handoffs,
            input_guardrails=email_guardrail,
        )

        with trace("Protected Automated SDR"):
            results = await Runner.run(careful_sales_manager, sales_manager_message)
            print(f"Guarded sales email:\n{results.final_output}")
